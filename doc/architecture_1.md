...
│   └── appimage/             # 她未来用于打包 AppImage 的构建脚本
...
为了让你们合作无间，架构设计中必须明确以下原则：

1. **绝对路径禁令：** 代码中绝不出现 `/Users/xxx` 或 `/home/xxx`。所有涉及文件读写（如数据库位置、日志位置）全部通过 `internal/config` 模块按相对路径或环境变量动态生成。
2. **网络层解耦：** Go 代码只通过 `http://127.0.0.1:11434` 与 Ollama 通信。在你的 Docker 中，你可以配置网络让其指向宿主机；在她的 Kali 中，这就直接指向她本地的 Ollama 服务。代码一行都不用改。
